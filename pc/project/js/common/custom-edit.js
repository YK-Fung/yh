define(['jquery'], function($){
	var customEdit = function(targetEl, moveRange, moveEl){
		var obj = {
			// 鼠标点击的目标
			'targetEl': targetEl,
			// 移动的范围
		...
		// 删除按钮
		$('.wrapper').on('mousedown', '.module .btn-del', function(event){
		...
		// 移动模块
		// 鼠标按下
		$('.wrapper').on('mousedown', obj.targetEl, function(event){
			var e = event || window.event;
			// 记录当前dom
			obj.source = $(this);
			// 鼠标按下，改为true
			...
			// 获取内容
			obj.moduleHtml = obj.source.clone();
	...
	var _customEdit = new customEdit('.module-ctn', window, '.rectangle');
});