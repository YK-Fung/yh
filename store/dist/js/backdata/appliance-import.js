define(['jquery', 'datetime' ,'table', 'table-tool', 'table-export', 'tableExport', 'zh'], function($){
	...
	$(document).on('click', '.stop-use', function() {
		obj = $(this);
		$('.pop-stop-use .text-primary').text('停用');
		$('.pop-stop-use .text-muted').text('确定停用吗？');
		$('.pop-stop-use .btn-yes').attr('data','stop-use');
	...
	$(document).on('click', '.start-use', function() {
		obj = $(this);
		$('.pop-stop-use .text-primary').text('启用');
		$('.pop-stop-use .text-muted').text('确定启用吗？');
		$('.pop-stop-use .btn-yes').attr('data','start-use');
		$('.pop-stop-use').modal('show');
	});

	//确定停用
	$('.pop-stop-use .btn-yes').click(function() {
		if($(this).attr('data') == 'stop-use'){
			obj.text('启用');
			obj.removeClass('red stop-use').addClass('blue start-use');
		}
		else if ($(this).attr('data') == 'start-use') {
			obj.text('停用');
			obj.removeClass('blue start-use').addClass('red stop-use');
		}
	});
