...
(function ($) {
    ...
    BootstrapTable.prototype.onSearch = function (event) {
        ...
        if (text === this.searchText&&$(event.currentTarget)[0].tagName=="INPUT") {
            return;
        }
        if($(event.currentTarget)[0].tagName=="INPUT") {
    ...
    BootstrapTable.prototype.initServer = function (silent, query, url) {
        ...
        if(this.options.sortable){
            if(this.options.sortName==undefined||this.options.sortName==null||this.options.sortName.toString().trim().length==0){
                var columns=this.options.columns[0];
                for(var i=0; i< columns.length; i++){
                    if(columns[i].loadingSort){
                        var sortName = "";
                        var colName=columns[i].field.trim();
                        for (var i = 0; i < colName.length; i++) {
                            var c = colName.substring(i, i + 1);
                            sortName += /[A-Z]/.test(c) ? "_" + c.toLowerCase() : c;
                        }
                        data["orderBy"] = sortName+ " " + this.options.sortOrder;
                        break;
                    }
                }
            }else {
                var sortName = "";
                for (var i = 0; i < this.options.sortName.length; i++) {
                    var c = this.options.sortName.substring(i, i + 1);
                    sortName += /[A-Z]/.test(c) ? "_" + c.toLowerCase() : c;
                }
                data["orderBy"] = sortName;
                if (this.options.sortOrder !== undefined && this.options.sortOrder.trim() != "") {
                    data["orderBy"] = data["orderBy"] + " " + this.options.sortOrder
                }
            }
        }
        $.extend(data, query || {});
