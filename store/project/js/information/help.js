define(['jquery','ZeroClipboard','move-el-help'],function($,ZeroClipboard){
	...
	$(document).on('click', '#comfirm-del',function() {
		...
		$('.box input').each(function() {
			if($(this).hasClass('active')){
				if($(this).hasClass('two') && $(this).parent().parent().children('.box').length == 1){
	...
	$(document).on('click', '.glyphicon-plus-sign', function() {
		...
		if(boxDOm){
			if($('.box input.active').hasClass('one')){
				//选中一级：在其内部最下方添加二级
				...
				$('.box input.active').parent('.box').append('<div class="box"><input type="text" class="name two" value="" placeholder="新建文档"></div>');
				//聚焦
				$('.box input.active').parent('.box').find('div.box:last input').focus();
			}else if($('.box input.active').hasClass('two')){
				//选中二级：在其下方添加二级
				$('.box input.active').parent('.box').after('<div class="box"><input type="text" class="name two" value="" placeholder="新建文档"></div>');
				//聚焦
				$('.box input.active').parent('.box').next().children('input').focus();
			}
		}else {
			// 未选中 在最下方添加一级
			$('.list').append('<div class="box"><input type="text" class="name one" value="" placeholder="新建文件夹"></div>');
	...
	var _Edit = new edit('.box input',document,'.move');
	//拖动完成ajax
